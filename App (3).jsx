...
/* ─── AI CLASSIFIER (keyword-based NLP simulation) ──────────────── */
const aiClassify = (description) => {
  const text = description.toLowerCase();
  let category = "facility";
  let priority = "Medium";

  if (/water|leak|pipe|flood|burst|drip|sewage|drain|blockage|overflow/.test(text)) {
    category = /sewage|drain|blockage|overflow|smell|sewer/.test(text) ? "sewage" : "water";
  } else if (/electric|power|light|outage|fault|spark|wire|trip|blackout/.test(text)) {
    category = "electricity";
  } else if (/pothole|road|crack|bump|tar|asphalt|pavement/.test(text)) {
    category = "pothole";
  } else if (/park|bench|facility|building|broken|vandal|graffiti|playground/.test(text)) {
    category = "facility";
  }

  const urgentWords = /danger|urgent|critical|emergency|hazard|injury|accident|flood|burst|fire|explosion|collapse/;
  const highWords   = /large|major|severe|bad|serious|significant|multiple|week|days|road/;
  const lowWords    = /small|minor|slight|little|tiny|crack/;

  if (urgentWords.test(text)) priority = "Critical";
  else if (highWords.test(text)) priority = "High";
  else if (lowWords.test(text)) priority = "Low";
  else priority = "Medium";

  const confidence = 0.75 + Math.random() * 0.22;
  return { category, priority, confidence: parseFloat(confidence.toFixed(4)) };
};
...
function MapCanvas({ complaints, selected, onSelect, height=280 }) {
  const ref = useRef(null);
  const [tip, setTip] = useState(null);
  const bounds = { minLat:-26.35, maxLat:-26.15, minLng:27.75, maxLng:28.15 };
  const xy = (lat,lng,W,H) => ({
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle="#C8DCE8"; ctx.lineWidth=0.5;
    for (let i=0;i<14;i++) {
      ctx.beginPath(); ctx.moveTo(i*W/14,0); ctx.lineTo(i*W/14,H); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0,i*H/10); ctx.lineTo(W,i*H/10); ctx.stroke();
    }
    [[[-26.30,27.75],[-26.30,28.15]],[[-26.22,27.75],[-26.22,28.15]],[[-26.35,27.95],[-26.15,27.95]]].forEach(road => {
      ctx.strokeStyle="#B8D0DC"; ctx.lineWidth=2.5; ctx.beginPath();
      road.forEach(([la,lg],i) => { const p=xy(la,lg,W,H); i===0?ctx.moveTo(p.x,p.y):ctx.lineTo(p.x,p.y); });
      ctx.stroke();
    });

    complaints.forEach(comp => {
      const cat = CATEGORIES.find(k=>k.id===comp.category);
      if (!cat) return;
      const p = xy(comp.lat||(-26.2+Math.random()*.1), comp.lng||(27.9+Math.random()*.2), W, H);
      const isSel = selected===comp.id;
      // Heat glow
      const grad = ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,isSel?30:20);
      grad.addColorStop(0, cat.color+"99");
      grad.addColorStop(1, cat.color+"00");
      ctx.beginPath(); ctx.arc(p.x,p.y,isSel?30:20,0,Math.PI*2);
      ctx.fillStyle=grad; ctx.fill();
      // Pin
      ctx.beginPath(); ctx.arc(p.x,p.y,isSel?14:10,0,Math.PI*2);
      ctx.fillStyle=cat.color; ctx.shadowColor=cat.color; ctx.shadowBlur=isSel?12:5; ctx.fill(); ctx.shadowBlur=0;
      ctx.fillStyle="#fff"; ctx.font=`${isSel?12:10}px sans-serif`;
      ctx.textAlign="center"; ctx.textBaseline="middle";
      ctx.fillText(cat.icon,p.x,p.y);
    });
  }, [complaints, selected]);
  ...
  const getHit = (e) => {
    ...
    return complaints.find(comp => {
      const p=xy(comp.lat||(-26.2),comp.lng||(28.0),c.width,c.height);
      return Math.hypot(mx-p.x,my-p.y)<18;
    })||null;
  };

  return (
    <div style={{ position:"relative", borderRadius:12, overflow:"hidden", border:`1px solid ${C.border}` }}>
      <canvas ref={ref} width={600} height={height} style={{ width:"100%", height:"auto", cursor:"crosshair", display:"block" }}
        onMouseMove={e=>setTip(getHit(e)?{c:getHit(e),x:e.nativeEvent.offsetX,y:e.nativeEvent.offsetY}:null)}
        onMouseLeave={()=>setTip(null)}
        onClick={e=>{const h=getHit(e);if(h)onSelect(h.id);}} />
      {tip && (
        <div style={{ position:"absolute",left:tip.x+12,top:tip.y-44,background:C.navy,color:"#E2E8F0",padding:"7px 11px",borderRadius:8,fontSize:12,pointerEvents:"none",whiteSpace:"nowrap",zIndex:10 }}>
          <strong style={{color:"#fff"}}>{tip.c.id||tip.c.ref_id}</strong> — {CATEGORIES.find(k=>k.id===tip.c.category)?.label}<br/>
          <span style={{color:"#94A3B8"}}>{tip.c.address||"Unknown location"}</span>
        </div>
      )}
      <div style={{ position:"absolute",bottom:6,left:6,display:"flex",gap:4,flexWrap:"wrap" }}>
        {CATEGORIES.map(c=>(
          <div key={c.id} style={{ background:"rgba(255,255,255,.92)",borderRadius:5,padding:"1px 7px",fontSize:10,display:"flex",alignItems:"center",gap:3,border:`1px solid ${c.color}44` }}>
            <span style={{fontSize:9}}>{c.icon}</span><span style={{color:c.color,fontWeight:600}}>{c.label}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ minHeight:"100vh",background:`linear-gradient(145deg,${C.navy} 0%,${C.navyMid} 60%,#1E3A5F 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ width:"100%",maxWidth:460 }}>
        ...
        <div style={{ textAlign:"center",marginBottom:32 }}>
          ...
          <p style={{ margin:"8px 0 0",color:"rgba(255,255,255,.55)",fontSize:14 }}>Johannesburg Municipal Services</p>
        </div>
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0" }}>
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10,opacity:.6 }}>Johannesburg Municipal Services</div>
            </div>
          </div>
        ...
        {!showForm&&view==="dashboard"&&(
          <div>
            ...
            <div style={{ background:"#fff",borderRadius:14,padding:20,border:`1px solid ${C.border}` }}>
              ...
              <MapCanvas complaints={complaints} selected={selected} onSelect={setSelected}/>
            </div>
          </div>
        )}
        ...
        {!showForm&&view==="map"&&(
          <div>
            ...
            <div style={{ background:"#fff",borderRadius:14,padding:20,border:`1px solid ${C.border}` }}>
              ...
              <MapCanvas complaints={complaints} selected={selected} onSelect={setSelected} height={340}/>
            </div>
          </div>
        )}
...
/* ─── CITIZEN SUBMIT FORM ───────────────────────────────────────── */
function CitizenSubmitForm({ onSubmit, onClose }) {
  ...
  return(
    <div>
      ...
      {step===2&&(
        <div>
          <label style={{fontSize:12,fontWeight:600,color:C.textMuted,display:"block",marginBottom:6,textTransform:"uppercase"}}>Description *</label>
          <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Describe the issue in detail…" rows={4} style={{width:"100%",borderRadius:10,border:`1.5px solid ${C.border}`,padding:"10px 12px",fontSize:14,resize:"vertical",boxSizing:"border-box",fontFamily:"inherit"}}/>
          <label style={{fontSize:12,fontWeight:600,color:C.textMuted,display:"block",margin:"14px 0 6px",textTransform:"uppercase"}}>Street Address *</label>
          <div style={{position:"relative"}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:18}}>
            ...
            <button disabled={!desc||!addr} onClick={()=>setStep(3)} style={{flex:2,padding:11,background:(!desc||!addr)?C.border:`linear-gradient(135deg,${C.blue},${C.teal})`,color:(!desc||!addr)?"#9CA3AF":"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:600,cursor:(!desc||!addr)?"not-allowed":"pointer"}}>Continue →</button>
          </div>
        </div>
      )}
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
        ...
        {view==="map"&&(
          <div>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:20,border:`1px solid ${C.border}`}}>
              <MapCanvas complaints={complaints.map(c=>({...c,id:c.ref_id,lat:parseFloat(c.latitude)||(-26.2+Math.random()*.15),lng:parseFloat(c.longitude)||(27.9+Math.random()*.2)}))} selected={selected} onSelect={setSelected} height={400}/>
            </div>
            {selected&&(()=>{
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
        ...
        {view==="map"&&(
          <div>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:20,border:`1px solid ${C.border}`}}>
              <MapCanvas complaints={assignments.map(a=>({...a,id:a.ref_id,lat:parseFloat(a.latitude)||(-26.2+Math.random()*.1),lng:parseFloat(a.longitude)||(27.9+Math.random()*.2)}))} selected={null} onSelect={()=>{}} height={360}/>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
        ...
        {view==="map"&&(
          <div>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:20,border:`1px solid ${C.border}`}}>
              <MapCanvas complaints={escalations.map(e=>({...e,id:e.ref_id,lat:parseFloat(e.latitude)||(-26.2+Math.random()*.1),lng:parseFloat(e.longitude)||(27.9+Math.random()*.2)}))} selected={null} onSelect={()=>{}} height={380}/>
            </div>
          </div>
        )}
      </div>
