...
function LiveTrackingMap({ complaints=[], technicians=[], selected, onSelect, height=320, title="Live Complaint Map" }) {
  ...
  const W = 660, H = Math.max(200, height);
  ...
  // Safe fallback coords — spreads pins across the map when GPS is null
  const fallbackCoord = (item, idx) => {
    const lat = parseFloat(item.lat || item.latitude);
    const lng = parseFloat(item.lng || item.longitude);
    return {
      lat: (isNaN(lat) || lat === 0) ? (-26.22 + (idx * 0.031) % 0.22) : lat,
      lng: (isNaN(lng) || lng === 0) ? (27.94  + (idx * 0.053) % 0.20) : lng,
    };
  };
  ...
  const toggleStatus = s => setVisibleStatuses(prev => {
  ...
  const handlePinClick = c => {
    if (onSelect) onSelect(c.id || c.ref_id);
    const tech = technicians.find(t => t.complaint_id === (c.dbId || c.id));
    setFooterMsg(tech
      ? `${c.id||c.ref_id} — ${tech.name} is ${tech.status_label||"en route"}`
      : `${c.id||c.ref_id} — ${c.status} · ${c.priority} priority`
    );
  };

  // Teardrop pin path
  const pin = (cx, cy, sc=1) =>
    `M${cx},${cy+16*sc} C${cx-13*sc},${cy+4*sc} ${cx-13*sc},${cy-13*sc} ${cx},${cy-13*sc} C${cx+13*sc},${cy-13*sc} ${cx+13*sc},${cy+4*sc} ${cx},${cy+16*sc}Z`;

  // Static city block rectangles (all absolute, no H-dependent math)
  const blocks = [
    ...
    [20,145,195,80],[220,145,160,80],[390,145,250,80],
    [20,240,195,80],[220,240,160,80],[390,240,250,80],
  ];

  // Road data as lat/lng pairs
  const mainRoads = [
  ...
  const placeLabels = [
    ["Emalahleni", -26.26, 27.88],
    ["CBD", -26.20, 27.99],
    ["Highveld", -26.11, 28.06],
    ["Witbank Ext", -26.11, 28.10],
    ["Del Judor", -26.16, 27.87],
    ["Hlalanikahle", -26.32, 28.13],
  ];
  const streetLabels = [
    ["Mandela Dr",    -26.22, 27.86],
    ["OR Tambo St",   -26.28, 27.86],
    ["Walter Sisulu", -26.15, 27.86],
  ];
  ...
  return (
    <div style={{ borderRadius:14, overflow:"hidden", border:`1px solid ${C.border}`, background:"#fff", fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", borderBottom:`1px solid ${C.border}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:C.textMuted }}>Real-time status &amp; technician location</div>
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"6px 14px", borderBottom:`1px solid ${C.border}`, background:C.slateLight, flexWrap:"wrap" }}>
        ...
        {LEGEND_STATUSES.map(s => (
          <label key={s} style={{ display:"flex", alignItems:"center", gap:4, cursor:"pointer", userSelect:"none" }}>
            <input type="checkbox" checked={visibleStatuses.has(s)} onChange={() => toggleStatus(s)} style={{ margin:0 }}/>
            <span style={{ width:8, height:8, background:STATUS_COLORS[s]||"#888", borderRadius:"50%", display:"inline-block" }}/>
      ...
      {/* ── Map SVG ── */}
      <div style={{ position:"relative", background:"#EDF4F8", lineHeight:0 }}>
        <svg ref={svgRef} width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display:"block", cursor:"crosshair" }}>

          {/* Background */}
          <rect x="0" y="0" width={W} height={H} fill="#EDF4F8"/>
          ...
          {/* Main roads */}
          {mainRoads.map((road,ri)=>{
            const [a,b] = road.map(([la,ln])=>toXY(la,ln));
            return (
              <g key={`mr${ri}`}>
                <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#fff"    strokeWidth="7" opacity="0.7"/>
                <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#B8D0DC" strokeWidth="5" opacity="0.5"/>
              </g>
            );
          })}

          {/* Secondary roads */}
          {secRoads.map((road,ri)=>{
            const [a,b] = road.map(([la,ln])=>toXY(la,ln));
            return <line key={`sr${ri}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#C8DCE8" strokeWidth="2" opacity="0.6"/>;
          })}

          {/* City blocks */}
          {blocks.map(([bx,by,bw,bh],i)=>(
            <rect key={`blk${i}`} x={bx} y={by} width={bw} height={bh} rx="3" fill="#C8DCE8" opacity="0.22"/>
          ))}

          {/* Place labels */}
          {placeLabels.map(([name,la,ln])=>{
            const p = toXY(la,ln);
            return <text key={name} x={p.x} y={p.y} textAnchor="middle" fontSize="9" fill="#90A4B4" fontFamily="sans-serif" fontWeight="600">{name}</text>;
          })}

          {/* Street labels */}
          {streetLabels.map(([name,la,ln])=>{
            const p = toXY(la,ln);
            return <text key={name} x={p.x+4} y={p.y-3} fontSize="8" fill="#7090A4" fontFamily="sans-serif">{name}</text>;
          })}

          {/* Technician scan lines */}
          {technicians.map((tech,ti)=>{
            ...
            return (
              <line key={`sl${ti}`} x1={tp.x} y1={tp.y} x2={cp.x} y2={cp.y-12}
                stroke={tech.color||"#378ADD"} strokeWidth="1.2" strokeDasharray="5 3" opacity="0.55"/>
            );
          })}
          ...
          {visible.map((c,i)=>{
            ...
            const active = c.status==="In Progress"||c.status==="Escalated";
            return (
              <g key={c.id||c.ref_id||i} style={{cursor:"pointer"}}
                onClick={()=>handlePinClick(c)}
                onMouseEnter={()=>setTooltip({c,x,y})}
                onMouseLeave={()=>setTooltip(null)}>
                {active && <circle cx={x} cy={y} r="18" fill={color} opacity="0.18"/>}
                <circle cx={x} cy={y-2} r={isSel?20:13} fill={color} opacity="0.12"/>
                <path d={pin(x,y,sc)} fill={color} stroke="#fff" strokeWidth="1.5"/>
                <circle cx={x} cy={y-2*sc} r={4.5*sc} fill="white" opacity="0.9"/>
                <circle cx={x} cy={y-2*sc} r={2*sc}   fill={color}/>
                <text x={x} y={y+30*sc} textAnchor="middle" fontSize="8" fill={C.textMuted} fontFamily="sans-serif">{c.id||c.ref_id}</text>
              </g>
            );
          })}
          ...
          {technicians.map((tech,ti)=>{
            ...
            return (
              <g key={`td${ti}`} style={{cursor:"pointer"}}
                ...
                <text x={x} y={y} textAnchor="middle" dominantBaseline="central" fontSize="8" fill="white" fontFamily="sans-serif" fontWeight="700">T</text>
              </g>
            );
          })}
          ...
          {tooltip && (()=>{
            ...
            const cat   = CATEGORIES.find(k=>k.id===c.category);
            const color = STATUS_COLORS[c.status]||"#888";
            const tx    = Math.min(x+14, W-200);
            const ty    = Math.max(y-72, 6);
            const addr  = String(c.address||"").slice(0,28);
            return (
              <g>
                <rect x={tx} y={ty} width="192" height="58" rx="8" fill="white" stroke="#E2E8F0" strokeWidth="1"/>
                <rect x={tx} y={ty} width="4"   height="58" rx="2" fill={color}/>
                <text x={tx+12} y={ty+15} fontSize="11" fontWeight="700" fill={C.text}    fontFamily="sans-serif">{String(c.id||c.ref_id)}</text>
                <text x={tx+12} y={ty+28} fontSize="10"                  fill={C.textMuted} fontFamily="sans-serif">{cat?.label||c.category}</text>
                <text x={tx+12} y={ty+41} fontSize="10" fontWeight="600" fill={color}      fontFamily="sans-serif">{c.status} · {c.priority}</text>
                <text x={tx+12} y={ty+54} fontSize="9"                   fill="#94A3B8"    fontFamily="sans-serif">{addr}{addr.length===28?"…":""}</text>
              </g>
            );
          })()}
        </svg>
        ...
        {/* Technician card */}
        {techCard && (
          <div style={{ position:"absolute", top:8, left:8, background:"#fff", border:`1px solid ${C.border}`, borderRadius:10, padding:"9px 13px", fontSize:12, minWidth:170, boxShadow:"0 4px 16px rgba(0,0,0,.1)", zIndex:10 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <span style={{ width:10, height:10, borderRadius:"50%", background:techCard.color||"#378ADD", display:"inline-block" }}/>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 14px", borderTop:`1px solid ${C.border}`, background:"#fff" }}>
        <div style={{ fontSize:11, color:C.textMuted }}>📍 {visible.length} complaint{visible.length!==1?"s":""} plotted</div>
        <div style={{ fontSize:11, fontWeight:600, color:footerMsg?"#D97706":C.textMuted }}>{footerMsg||"Emalahleni Local Municipality"}</div>
          ...
          <div style={{ background:"#fff",borderRadius:18,padding:30,maxWidth:400,width:"90%" }}>
            ...
            <div style={{ display:"flex",gap:10,marginTop:22 }}>
              ...
              <button onClick={async()=>{
                ...
                await fetch(`${API}/api/escalations/request-review`,{
                  method:"POST",
...
function AdminDashboard({ user, onLogout }) {
  ...
  const escalateCouncillor=async(complaint)=>{
    ...
    }catch(err){notify("Network error — could not escalate","error");}
  };
                  ...
                  <div key={s} style={{marginBottom:10}}>
                    ...
                    <div style={{height:10,background:"#F1F5F9",borderRadius:5,overflow:"hidden"}}>
                      <div style={{height:"100%",background:
                        ...
                        s==="Escalated"?"#DC2626":
                        s==="Assigned"?"#0EA5E9":
                        s==="Classified"?"#8B5CF6":"#F59E0B",borderRadius:5,width:`${complaints.length?Math.round(count/complaints.length*100):0}%`,transition:"width .5s"}}/>
                    </div>
                  </div>
                ))}
              </div>
